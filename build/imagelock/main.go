...
// KAI Resource Management release needs: one ImageLock document per profile and
// platform, covering the images this repository publishes. The bundled
// kai-scheduler subchart locks its own.
//
// Run it through the Makefile: `make image-lock VERSION=vX.Y.Z`, or
// `make image-lock-check` for the offline check that runs on every pull request.
// See README.md.
package main
...
// releaseVersion rejects "latest" and other moving references, which would pin a
// lock to whatever happened to be tagged at generation time.
var releaseVersion = regexp.MustCompile(`^v?[0-9]+\.[0-9]+\.[0-9]+`)

type options struct {
	chart string
	// version is both the tag the chart renders with and the version the lock
	// records, so it has to be the tag that was actually published.
	version string
	// registry is where this release publishes, and so what is locked. It is
	// passed in because values.yaml still holds the local build default here.
	registry  string
	platforms []platform
	profiles  []profile
	outDir    string
	// helmBin is empty for "helm".
	helmBin string
	// verifyOnly renders and classifies without reaching a registry or writing a
	// file: the pull request check.
	verifyOnly bool
}
...
func (o options) validate() error {
	...
	if o.verifyOnly {
		// Verification never names a file, so it needs no release version.
		return nil
	}
	if o.version == "" {
...
// generate writes nothing until every digest is in hand, so a registry failure
// leaves no partial set of locks behind.
func generate(ctx context.Context, opts options) error {
	...
	for _, prof := range opts.profiles {
		...
		// #nosec G706 -- tainted only because the names come from `helm template`;
		// this line is what shows a reviewer which images a release will lock.
